  ...
  try {
    ...
    if (!name || !workouts) {
      return res.status(400).json({
        ...
        details: [
          ...(name ? [] : [{ field: 'name', message: 'Schedule name is required' }]),
          ...(workouts ? [] : [{ field: 'workouts', message: 'Workouts array is required' }])
        ]
      });
    }
    ...
    if (errorMessage.includes('required') || errorMessage.includes('must have')) {
      return res.status(400).json({
        success: false,
        error: errorMessage,
